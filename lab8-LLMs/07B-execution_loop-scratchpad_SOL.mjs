...
async function runAgentTurn(userInput, maxIterations = 6) {
  // Local scratchpad for this specific turn.
  // It includes previous visible conversation, plus internal actions and observations.
  const turnMessages = [
    {
      role: "system",
      content: AGENT_PROMPT,
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (finalAnswer) {
      ...
      // Store only the visible user request and final assistant answer.
      messages.push({
        role: "user",
        content: userInput,
      });
...
console.log("Mini agent 07B: execution loop with local scratchpad started.");
