    ...
    onDelivery: ({agentId, parcels}) => {
        for ( let p of parcels ) {
            if ( p.deliveredBy && p.deliveredBy != p.initiallyPickedUpBy ) {
                ...
                // Reward the agent for delivering a parcel initially picked up by another agent
                emitReward(agentId, BONUS_REWARD, 'delivered a parcel initially picked up by another agent');
            }
        }
    }
} );
